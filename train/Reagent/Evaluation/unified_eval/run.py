"""
Unified multi-dataset evaluation runner.
Automatically configures tools and settings based on dataset.
"""
import argparse
import json
import os
...
from prompt_generator import get_dataset_config
import time
import math
...
if __name__ == "__main__":
    ...
    parser.add_argument("--judge_engine", type=str, default="deepseekchat", 
                       choices=["deepseekchat", "geminiflash"], 
                       help="LLM engine for auto judging")
    parser.add_argument("--eval_data_dir", type=str, default="../eval_data", 
                       help="Directory containing evaluation data files")
    parser.add_argument("--planning_ports", type=int, nargs='+', 
                       default=[6001, 6002, 6003, 6004, 6005, 6006, 6007, 6008],
                       help="List of vLLM server ports")
    ...
    try:
        ...
        print(f"Tools: {dataset_config['tools']}")
        print(f"Max LLM calls: {dataset_config.get('max_llm_calls', 100)}")
        print(f"{'='*80}\n")
    ...
    model_dir = os.path.join(output_base, f"{model_name}_unified")
    ...
    # Load data file
    data_filename = dataset_config.get('data_file', f"{args.dataset}.jsonl")
    data_filepath = os.path.join(args.eval_data_dir, data_filename)
    ...
    for rollout_idx in range(1, roll_out_count + 1):
        ...
        for item in items:
            ...
            # Handle file attachments (only for rollout_idx == 1 to avoid duplication)
            if rollout_idx == 1 and dataset_config.get('has_file_attachments', False):
                if item.get("file_name") and item.get("file_name").strip():
                    file_base_path = os.getenv(
                        dataset_config.get('file_base_path_env', 'FILE_BASE_PATH'),
                        dataset_config.get('default_file_base_path', '')
                    )
                    file_name = os.path.join(file_base_path, item.get("file_name"))
    ...
    if not tasks_to_run_all:
        ...
        # Initialize agent with dataset configuration
        test_agent = MultiTurnReactAgent(
            llm=llm_cfg,
            function_list=dataset_config['tools'],

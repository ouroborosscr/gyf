...
class AgentExecutionEngine:
    def __init__(
        ...
        env_args=None,
        max_workers=128,  # The number of concurrent env operations
    ...
    async def run_agent_trajectory_async(self, idx, application_id, seed=0, mode="Text", **kwargs):
        ...
        skip_agent_reset = meta_info.get("skip_agent_reset", False)
        
        if not skip_agent_reset:
            agent.reset()
        ###################################################
        ...
        if mode == "Text":
            ...
            token_result = {
                ...
                "metrics": {
                    ...
                    **self._extract_reward_metadata(trajectory),
                },
            }
            return token_result
                ...
                if current_prompt_ids[: len(accumulated_sequence)] != accumulated_sequence:
                    ...
                    for i, (expected, actual) in enumerate(zip(accumulated_sequence, prefix, strict=False)):
                        if expected != actual:
                            diff_pos = i
                            break
                    # 仅对特定模式进行硬纠正：Expected [271, 151645], Got [151645, 198]
                    if (
                        diff_pos is not None
                        and len(accumulated_sequence) - diff_pos >= 2
                        and len(prefix) - diff_pos >= 2
                        and accumulated_sequence[diff_pos] == 271
                        and accumulated_sequence[diff_pos + 1] == 151645
                        and prefix[diff_pos] == 151645
                        and prefix[diff_pos + 1] == 198
                    ):
                        logger.warning(
                            f"Token mismatch pattern (271,151645)->(151645,198) detected at pos {diff_pos}, "
                            "force-aligning delta_prompt+completion instead of zeroing masks."
                        )
                        delta_prompt = current_prompt_ids[len(accumulated_sequence):]
                        response_tokens.extend(delta_prompt + current_completion_ids)
                        response_masks.extend([0] * len(delta_prompt) + [1] * len(current_completion_ids))
                        accumulated_sequence = current_prompt_ids + current_completion_ids
                        # 不设置 is_valid_trajectory=False，不 break
                    else:
...
class AsyncAgentExecutionEngine(AgentExecutionEngine):
    ...
    def _extract_reward_metadata(self, trajectory):
        """Extract reward分解信息 from the last step of the trajectory."""
        reward_metadata = {}
        
        if trajectory.steps:
            last_step = trajectory.steps[-1]
            if hasattr(last_step, 'info') and last_step.info:
                metadata = last_step.info.get('metadata', {})
                
                # 提取reward相关的metadata
                if 'base_reward' in metadata:
                    reward_metadata['base_reward'] = metadata['base_reward']
                if 'reward_model_score' in metadata:
                    reward_metadata['reward_model_score'] = metadata['reward_model_score']
                if 'final_reward' in metadata:
                    reward_metadata['final_reward'] = metadata['final_reward']

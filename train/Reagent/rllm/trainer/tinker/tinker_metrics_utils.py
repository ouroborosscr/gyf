...
def compute_env_metrics(episodes: list[Episode]) -> dict:
        ...
        for traj in episode.trajectories:
            ...
            for step in traj.steps:
                ...
                if hasattr(step, "info") and step.info:
                    ...
                    # 新增：从step.info中提取metadata
                    metadata = step.info.get("metadata", {})
                    if "base_reward" in metadata:
                        base_rewards.append(metadata["base_reward"])
                    if "reward_model_score" in metadata:
                        rm_scores.append(metadata["reward_model_score"])
                    if "final_reward" in metadata:
                        final_rewards.append(metadata["final_reward"])
    ...
    # 新增：添加分解的reward统计
    if base_rewards:
        metrics["reward/base_reward_mean"] = np.mean(base_rewards)
        metrics["reward/base_reward_std"] = np.std(base_rewards)
    if rm_scores:
        metrics["reward/rm_score_mean"] = np.mean(rm_scores)
        metrics["reward/rm_score_std"] = np.std(rm_scores)
    if final_rewards:
        metrics["reward/final_reward_mean"] = np.mean(final_rewards)
        metrics["reward/final_reward_std"] = np.std(final_rewards)

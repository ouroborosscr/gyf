...
def calc_elo_rating(contest_id: int, problem_status: dict[str, list[bool]], sorted_ratings: list[float], pass_n=None) -> tuple[int, float] | None:
    ...
    try:
        ...
        try:
            handle_set_standings = set(standings["result"]["rows"][i]["party"]["members"][0]["handle"] for i in range(len(standings["result"]["rows"])))

            handle_set_ratings = set(rating_changes["result"][i]["handle"] for i in range(len(rating_changes["result"])))

            handle_set = handle_set_standings.intersection(handle_set_ratings)

            standings["result"]["rows"] = [row for row in standings["result"]["rows"] if row["party"]["members"][0]["handle"] in handle_set]

            rating_changes["result"] = [change for change in rating_changes["result"] if change["handle"] in handle_set]
        ...
        percentile = get_percentile(left, sorted_ratings)
        return left, percentile
...
def format_grouped_contest_data(submissions: list[list[bool]], problem_ids: list[str]) -> list[tuple[int, dict[str, list[bool]]]]:
    ...
    for problem_id, submission in zip(problem_ids, submissions, strict=False):

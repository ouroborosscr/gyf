...
# 增加 tokenizer 参数
def build_grpo_dataset(tokenizer, target_date=TARGET_DATE, range_start=1, range_end=3000, num_samples=200):
    ...
    for _ in tqdm(range(num_samples), desc="正在抽取流量数据"):
        ...
        # 补全与 graph.py 一致的详细信息说明，去掉了原来“严格约束”的模拟词
...
15.history:这是一个由字母组成的字符串,记录了数据包的交互顺序。
...
请结合提供的工具，通过标准的工具调用格式(Tool Call)来报告可疑流量。
...
{flows_json}"""
        ...
        try:
            # 【核心逻辑】使用 Qwen / Transformers 官方的工具调用模版引擎
            final_prompt = tokenizer.apply_chat_template(
                messages,
                tools=tool_schemas,  # 直接将工具传入分词器
                tokenize=False,
                add_generation_prompt=True
            )
        except Exception as e:
            # 兼容性 Fallback：如果当前分词器不支持内置 tools 渲染，退化为在 System 提示中严格注入 JSON Schema
            tools_str = json.dumps(tool_schemas, ensure_ascii=False, indent=2)
            messages[0]["content"] += f"\n\n# Tools\nYou are equipped with the following tools. You must use JSON format to call a tool:\n{tools_str}"
            final_prompt = tokenizer.apply_chat_template(
                messages,
                tokenize=False,
                add_generation_prompt=True
    ...
    for comp, truth in zip(completions, ground_truth):
        ...
        if true_start is None:
            if is_susp_match and not pred_is_suspicious:
                # 完美：模型明确调用了工具，并判定 is_suspicious 为 False
                rewards.append(1.0)
            else:
                # 误报为有攻击，或者根本没按格式输出 is_suspicious 参数，直接扣分
                rewards.append(-1.0)
            continue
        ...
        if not pred_is_suspicious:
            # 漏报（模型判定为没攻击，或者缺漏了该参数）
            rewards.append(-1.0)
            continue
            
        if not (start_match and duration_match):
            # 报了攻击，但是没给全 start 和 duration 参数
            rewards.append(-1.0)
            continue
...
def logic_reward_func(completions, **kwargs):
    ...
    def evaluate_single(idx, comp):
        ...
        try:
            ...
            # 捕获 HTTP 错误（如上下文超长 400，或超时）
            if idx == 0 and not printed_this_step:
                print(f"\n❌ [RRM API 请求失败] HTTP 异常: {e}")
                # 如果是 400 错误，尝试打印服务器返回的错误详情
...
def main():
    ...
    try:
        ...
        # 修改后 (保证这一行在 tokenizer 实例化之后执行)
        dataset = build_grpo_dataset(tokenizer=tokenizer, target_date=TARGET_DATE, range_start=1, range_end=3000, num_samples=200)

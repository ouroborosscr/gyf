...
输出 PASS 或 FAIL（只输出一个词）。
"""


def llm_judge_step(tool: str, expect: str, result: Dict[str, Any]) -> str:
...
FINAL_VERDICT_PROMPT = """你是网络流量攻击审计专家。基于以下证据，给出最终判定。

【已审计的文档/工具】{audited_docs}
【全部证据】
{all_evidence}

请输出 JSON 格式的最终判定（注意只输出 JSON，不要任何其他文字）：
{{
  "is_attack": true/false,
  "attack_type": "...",
  "tool": "frp / nps / chisel / unknown",
  "matched_sub_skill": "frp_quic / frp_ws / ...",
  "confidence": 0.0-1.0,
  "key_evidence": ["...", "...", "..."],
  "recommended_actions": ["..."]
}}
...
def llm_final_verdict(evidence: list, audited_docs: list) -> Dict[str, Any]:
    """生成最终判定 JSON"""
    if LLM_MODE == "mock":
    ...
    prompt = FINAL_VERDICT_PROMPT.format(
        ...
        all_evidence=_format_evidence(evidence)
    )
    try:
        resp = _try_real_llm(prompt)
        # 提取 JSON

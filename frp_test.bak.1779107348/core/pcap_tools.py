...
def pcap_conversations(pcap_path: str, proto: str = "tcp", limit: int = 20) -> Dict[str, Any]:
    ...
    for line in r["stdout"].splitlines():
        ...
        # 注意 bytes 列可能含单位（如 "15 kB"），字段之间至少 2 个空格分隔
        if "<->" not in line:
            continue
        try:
            left, right = line.split("<->", 1)
            endpoint_a = left.strip()
            right = right.strip()
            ep_b_match = re.match(r"(\S+)\s+(.+)", right)
            if not ep_b_match:
                continue
            endpoint_b = ep_b_match.group(1)
            remainder = ep_b_match.group(2)
            
            # 按 2+ 个空格拆字段：["44 15 kB", "147 170 kB", "191 185 kB", "0.000000000", "132.6374"]
            fields = re.split(r"\s{2,}", remainder.strip())
            if len(fields) < 5:
                continue
            
            def parse_frames_bytes(s: str):
                """解析 '44 15 kB' 或 '147 170 kB' 这种格式"""
                parts = s.strip().split(None, 1)
                if not parts:
                    return 0, 0
                frames = int(parts[0].replace(",", ""))
                if len(parts) == 1:
                    return frames, 0
                bytes_str = parts[1].strip()
                num_m = re.match(r"([\d.]+)", bytes_str)
                if not num_m:
                    return frames, 0
                n = float(num_m.group(1))
                if "kb" in bytes_str.lower():
                    n *= 1024
                elif "mb" in bytes_str.lower():
                    n *= 1024 * 1024
                elif "gb" in bytes_str.lower():
                    n *= 1024 * 1024 * 1024
                return frames, int(n)
            
            frames_a, bytes_a = parse_frames_bytes(fields[0])
            frames_b, bytes_b = parse_frames_bytes(fields[1])
            total_f, total_b = parse_frames_bytes(fields[2])
            rel_start = float(fields[3])
            duration = float(fields[4])
            
            conversations.append({
                "endpoint_a": endpoint_a,
                "endpoint_b": endpoint_b,
                "frames_a_to_b": frames_a,
                "bytes_a_to_b": bytes_a,
                "frames_b_to_a": frames_b,

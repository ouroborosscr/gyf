...
def _try_real_llm(prompt: str) -> str:
    """调用真实 LLM"""
    from llm import llm
...
def llm_skill_gate(evidence: list, doc: Dict[str, Any], skill: Dict[str, Any]) -> str:
    ...
    try:
        resp = _try_real_llm(prompt).upper()
...
3. frp_tcp 和 frp_tcp_tls 在网络层无法区分（v0.50+ 默认 TLS）；只看见 TCP/TLS 但无 SNI 异常时优先 frp_tcp
4. frp_wss 必须看到「SNI 可疑域名 + Go JA3 + 443 端口」三者组合
5. frp_ws 必须看到 HTTP URI = /~!frp 才能确认；只有心跳 PASS 不够
...
FRP_SKILL_CATALOG = {
    "frp_tcp":     "TCP 默认模式（v0.50+ 默认启用 TLS）。识别：Go 默认 JA3 (2196848d... 或 20b27999...) + 30s 精确心跳 + 流量方向反转（resp/orig > 5）。无 hard fingerprint。",
    "frp_tcp_tls": "TCP + 强制 TLS 模式（transport.tls.force=true）。网络层与 frp_tcp 完全不可区分，识别特征同 frp_tcp。区别仅在服务端策略。",
    "frp_kcp":     "KCP over UDP 模式。识别：UDP 长会话 + 包大小分布中 MTU 饱和段（1280-2559 字节占比 > 60%）+ 30s 心跳。无 QUIC 握手（区别于 frp_quic）。",
    "frp_quic":    "QUIC 模式。★★★ 决定性指纹：QUIC ALPN = 'frp'（合法 QUIC 是 h3/hq-XX）。辅助：10s 短心跳。",
    "frp_ws":      "WebSocket 明文模式。★★★ 决定性指纹：HTTP URI = /~!frp（硬编码在 frp 源码 issue #5278）。辅助：30s 心跳，明文 HTTP 1.1 Upgrade。",
    "frp_wss":     "WebSocket over TLS 模式，必须前置 nginx/caddy 做 TLS 终结。识别：Go 默认 JA3 + 可疑 SNI（非主流域名/.test/动态 DNS）+ 自签证书 + 主动探查 nginx + 30s 心跳。无单一 hard fingerprint，需组合判定。",
}
